...
def get_top_chunks(query: str, top_k: int = 2):
    """Kullanıcının sorusuna en yakın top_k adet doküman parçasını veri tabanından getirir."""
    ...
    print(f"Veri tabanındaki {len(rows)} parça ile karşılaştırma yapılıyor...")

    # 4. Benzerlik Skorlarını Hesaplama ve Sıralama
    ranked_chunks = []
    for content, source, chunk_id, raw_embedding in rows:
        # Saklanan JSON metni vektöre (float listesi) dönüştürülüyor
...
def main():
    print("====================================================")
    print(" 3. Hafta - Adım 3: Geri Getirme (Retrieval) Testi  ")
    print("====================================================")

    # Test araması
    test_query = "Mavi ışık yanıp söndüğünde ne yapmalıyım?"
    
    print(f"Arama Başlatılıyor...")
    top_results = get_top_chunks(test_query, top_k=2)

    print("\n--- Geri Getirilen En Alakalı Parçalar (Top 2) ---")
    for idx, chunk in enumerate(top_results):
        print(f"\n[Sonuç #{idx+1}] Skor: {chunk['score']:.4f} | Kaynak: {chunk['source']} (Parça: {chunk['chunk_id']})")
        print(f"İçerik: {chunk['content']}")
        print("-" * 50)

    print("\nRetrieval Sandbox testi başarıyla tamamlandı!")
